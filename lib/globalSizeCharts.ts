export type SizeRow = {
  ...
  weight_min: number;
  weight_max: number;
  chest:      number;        // half-circumference in cm
  hips?:      number;        // half-circumference in cm (long clothing)
  shoulder?:  number;        // width in cm (t-shirt)
  length:     number;        // garment length in cm
};
