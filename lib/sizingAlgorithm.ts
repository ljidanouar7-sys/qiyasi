...
export function calculateSize(input: AlgoInput): SizeResult {
  const { niche, height, weight, katif, sadr, khasr, warek, size_chart } = input;

  if (!size_chart?.length) {
    return { size: "", status: "ok", alternatives: [] };
  }

  if (niche === "tshirt" || niche === "t_shirt") {
    return getTshirtSize(size_chart, height, weight, katif, khasr);
  }
